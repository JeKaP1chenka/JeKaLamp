...
void fireTick(byte scale, int len) {
  ...
  if (loading) {
    ...
    deltaHue = map(deltaValue, 8U, 168U, 8U,
                   84U);  // высота языков пламени должна уменьшаться не так
                          // быстро, как ширина
    step = map(255U - deltaValue, 87U, 247U, 4U,
               32U);  // вероятность смещения искорки по оси ИКС
    for (uint8_t j = 0; j < HEIGHT; j++) {
      shiftHue[j] =
          (HEIGHT - 1 - j) * 255 / (HEIGHT - 1);  // init colorfade table
    }
  ...
  for (uint8_t i = 0; i < WIDTH; i++) {
    for (uint8_t j = 0; j < len; j++) {
      leds[getPixelNumber(i, len - 1U - j)] = ColorFromPalette(
          paletteArr[1],
          qsub8(
              inoise8(i * deltaValue, (j + ff_y + random8(2)) * deltaHue, ff_z),
              shiftHue[j]),
          // 255U
          255U);
    }
  ...
  for (uint8_t i = 0; i < WIDTH / 8; i++) {
    if (trackingObjectPosY[i] > 3U) {
      leds[getPixelNumber(trackingObjectPosX[i], trackingObjectPosY[i])] =
          leds[getPixelNumber(trackingObjectPosX[i], 3U)];
      leds[getPixelNumber(trackingObjectPosX[i], trackingObjectPosY[i])]
          .fadeToBlackBy(trackingObjectPosY[i] * 2U);
    }
    ...
    if (!random8(step))
      trackingObjectPosX[i] =
          (WIDTH + (uint8_t)trackingObjectPosX[i] + 1U - random8(3U)) % WIDTH;
  }
  ff_y++;
  if (ff_y & 0x01) ff_z++;
}
